...
def __index_files(path, use_file=None):
    xlat = {
        "e164num/": SCHEMA_NAMESPACE + "e164num",
        "route/": SCHEMA_NAMESPACE + "route",
        "itad/": SCHEMA_NAMESPACE + "itad",
...
def test_policy(obj_type, name, mntner):
    ...
    elif obj_type in ["e164num"]:
        log.info("Checking e164num type")
        lis = find(["mnt-by"], {"@type": obj_type, "prefix": name})
        log.debug(lis)
...
def sanity_check(dom):
    "sanity check"
    ck = "PASS"
    #if dom.schema == "tripnet.e164num":
    #    prefix = dom.get("prefix")
    #    Lnet, Hnet, _ = e164range(e164)
    #    cidr_range = pretty_ip(Lnet) + "-" + pretty_ip(Hnet)
    #    file_range = dom.get("e164num")
    #    file_range = re.sub(r"\s+", "", file_range, flags=re.UNICODE)

    #    if cidr_range != file_range:
    #        log.error(
    #            "e164num range [%s] does not match: [%s]" % (file_range, cidr_range)
    #        )
    #        ck = "FAIL"
...
def get_args():
    ...
    parser_mroute = subparsers.add_parser(
        "match-routes", help="Match routes to e164nums"
    )
    _ = parser_mroute
